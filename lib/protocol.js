...
// exports.nameOfServiceProp = nameOfServiceProp; // Не используется
...
/**
 * Формирует массив правил опроса для одного счетчика 
 * @return {Array of Objects} - [{mid:'I', chan:'I1', <tarif,> buf, readfn, count },..]
 */
...
function read4Energy(buf, pollItem, assets) {
  ...
  return [
    { chan: 'EAP' + sut + tstr, value: round(res[0]) },
    { chan: 'EAM' + sut + tstr, value: round(res[1]) },
    { chan: 'ERP' + sut + tstr, value: round(res[2]) },
    { chan: 'ERM' + sut + tstr, value: round(res[3]) }
  ];
}
...
function parseAddress(buf) {
  if (buf.size < 5)
    throw { messsage: 'parseAddress: Expected buffer with long address (5 byte), received bytes: ' + buf.size };
